...
float* _hog_compute(unsigned char* src, float* hog, rectangle_t *roi, int cell_size)
{
  ...
  //若TRAIN为true，重新训练分类器
	if(TRAIN)
	{
		...
		for(int num=0; num<PosSamNO && getline(finPos,ImgName); num++)
		{
			...
			rectangle_t roi = { 0, 0, 64, 128 };
			_hog_compute(pImg_data, hog_des, &roi, 8);
